...
# Choose how many days you want to predict. Ideally within the next 2 weeks
num_days_to_predict = 14

last_days_scaled = scaled_data[-1095:] # using last 3 years as the data to predict (1095 days)

# Make predictions for each day in the future
new_predictions = []
current_batch = last_days_scaled.reshape((1, 1095, 1))
...
future_predictions = pd.DataFrame(index=future_dates, data=new_predictions, columns=['Predictions'])

plt.figure(figsize=(10, 5))
plt.title('Apple Stock Price Prediction using LSTM')
plt.xlabel('Date')
plt.ylabel('Price (USD)')
plt.plot(apple_data['Close'], label='Historical Closing Price', color='blue')
plt.plot(valid['Predictions'], label='Model Predictions', color='orange')
plt.plot(future_predictions['Predictions'], label='Future Predictions', color='green', linestyle='--')
